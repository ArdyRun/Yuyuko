...
const { GEMINI_API_KEY, OPENROUTER_API_KEY } = require("../../environment.js");
const { designatedChannelIds } = require("../../utils/config.js");
const fs = require("fs");
...
// Gemini for image analysis and novel suggestions (multimodal features)
...
// OpenRouter configuration for text chat
...
// Cleanup old user data every hour to prevent memory leaks
setInterval(() => {
  const cutoff = Date.now() - 24 * 60 * 60 * 1000; // 24 hours ago
  for (const [id, data] of userData.entries()) {
    if (data.lastInteraction && data.lastInteraction.getTime() < cutoff) {
      userData.delete(id);
    }
  }
...
async function handleTextConversation(message, prompt, userName, userId, userInfo, conversationHistory) {
  ...
  const contextInfo = `${userContext}${historyContext}${personalHistoryContext}${replyContext}`;
  const userMessageWithContext = `${contextInfo}\nUser berkata: "${prompt}"\n\nRespond as Ayumi:`;

  // Use OpenRouter for text chat (mandatory)
  let reply = await callOpenRouter(userSystemPrompt, userMessageWithContext);
...
async function handleAyumiCommand(message) {
  ...
  // These channel IDs are imported from utils/config.js for centralized management
